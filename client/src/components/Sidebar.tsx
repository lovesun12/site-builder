...
import type { Project as boolean, Message, Project, Version } from '../types';
...
const Sidebar = ({isMenuOpen , project ,setProject , isGenerating,setIsGenerating} : SidebarProps) => {
  ...
  return (
    <div className={`${isMenuOpen ? 'max-sm:w-0 overflow-hidden' : 'w-full'} h-full sm:max-w-sm rounded-xl bg-gray-900 border-gray-800 transition-all`}>
                            ...
                            <div key={ver.id} className='w-4/5 mx-auto my-2 p-3 rounded-xl bg-gray-800 text-gray-100 shadow flex flex-col gap-2'>
                                ...
                                <div className='flex items-center justify-between'>
                                    {project.current_version_index === ver.id ? (
                                        ...
                                        <button onClick={()=>handleRollback(ver.id)} className='px-3 py-1 rounded-md text-xs bg-indigo-700 hover:bg-indigo-600 text-white'>Roll back to this version</button>
                                    )}
                                    <Link target='_blank' to={`/preview/${project.id}/${ver.id}`} className='text-xs text-indigo-400 hover:text-indigo-300'>
            ...
            <form onSubmit={handleRevisions} className='m-3 relative'>
                <div className='flex items-center gap-2'>
                    ...
                    <button disabled={isGenerating || !input.trim()} className='absolute bottom-2.5 right-2.5 rounded-full bg-linear-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white transition-colors disabled:opacity-60'>
                        {isGenerating 
                        ? <Loader2 className='animate-spin size-7 p-.15 text-white'/>

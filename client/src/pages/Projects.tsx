...
import { ArrowBigDownDashIcon, EyeIcon, EyeOffIcon, FullscreenIcon, LaptopIcon, Loader2, MessageSquareIcon, SaveIcon, SmartphoneIcon, TabletIcon, XIcon } from 'lucide-react';
...
const Projects = () => {
  ...
  const fetchProject= async ()=>{
    try {
      ...
      toast.error(error?.response?.data?.message || 'Something went wrong while fetching the project');
      console.log(error);
  ...
  useEffect(()=>{
    if(session?.user && !isPending){
      ...
      navigate('/');
      toast.error('Please sign in to access the project');
    }
  },[session?.user])
      ...
      <div className='flex max-sm:flex-col sm:items-center gap-4 px-4 py-2 no-scrollbar'>
        ...
        <div >
          ...
          <div className='max-w-64 sm:max-w-xs'>
            ...
            <p className='text-xxs text-gray-400 -mt-0.5'>Previewing last saved version</p>
          </div>
        ...
        <div className='flex items-center justify-end gap-3 flex-1 text-xs sm:text-sm'>
          ...
          <button onClick={downloadCode} className='bg-linear-to-br from-blue-700 to-blue-600 hover:from-blue-600 hover:to-blue-500 text-white px-3.5 py-1 flex items-center gap-2 rounded sm:rounded sm:rounded-sm transition-colors'>
            <ArrowBigDownDashIcon size={16}/>Download
          </button>
          <button onClick={togglePublish} className='bg-linear-to-br from-blue-700 to-blue-600 hover:from-blue-600 hover:to-blue-500 text-white px-3.5 py-1 flex items-center gap-2 rounded sm:rounded sm:rounded-sm transition-colors'>
            {project.isPublished ? 
